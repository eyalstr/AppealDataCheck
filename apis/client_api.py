...
from utils.json_parser import get_first_request_id
import datetime
...
def fetch_case_details(case_id):
    ...
    try:
        ...
        log_and_print(f"🔎 Raw response status: {response.status_code}")
        log_and_print(f"🔎 Response text: {response.text[:50]}...")
